...
**good-readme** — writes, improves, or audits a README, checking every documented import, command and flag against the actual manifests and exports before it ships. Like gauntlet-loop it has no binary; it is global because every repo has a README and no per-repo trigger predicts when one needs work (D58). Vendored from [adewale/good-readme](https://github.com/adewale/good-readme) (MIT).
