...
fn build_draw_calls(scene: &Scene, camera: &Camera) -> Vec<DrawCall> { ... }
...
Run `scripts/generate_layout_tests.py` to auto-generate memory layout tests.
...
```bash
python scripts/generate_layout_tests.py src/types.rs > src/types_layout_tests.rs
python scripts/generate_layout_tests.py src/ --recursive > tests/layout_tests.rs
```
...
Rust iterators compile to the same code as hand-written loops. Prefer them
always — they express intent, enable fusion, and prevent off-by-one errors.
...
let visible: Vec<DrawCall> = entities.iter()
    ...
    .sorted_unstable_by_key(|e| e.material_id()) // sort by material to batch
