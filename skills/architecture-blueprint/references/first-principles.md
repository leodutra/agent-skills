...
The rationale layer of this skill: a pattern-free foundation carried here in full, with its derivation tree and pattern ledger. Every rule in SKILL.md and the other references is a consequence of it. The `(n)` tags on those rules point at the numbered principles below, `(frame)` at the proof-obligation frame and its currency ladder, `(A1–A4)` at the foundations, and `(ledger)` at the pattern ledger, which carries each pattern's derivation and the conditions under which it binds. (Keyword conventions: see SKILL.md.)

Use this file when:

- no decision rule covers the situation — derive the answer from the principles instead of importing a pattern;
- a rule is being justified or challenged — follow its tag here and argue from the derivation, not from the pattern's reputation;
...
## Index: where this skill applies each principle

| Principle | Applied in |
| --- | --- |
| Frame — currency ladder; an obligation must exist before it is assigned | SKILL.md core rule, Evolution Path, Avoid by default, Restraint check; `testing-and-governance.md` (rung 2: tests, fitness functions; rung 3: ADRs, READMEs, `AGENTS.md`) |
| A1 — reasoners are bounded (human or machine) | SKILL.md North Star (comprehensibility, AI navigability); `testing-and-governance.md` AI context file |
| A2 — reasoning contexts are not shared | ADRs as the only carrier of a decision to later maintainers (`testing-and-governance.md`) |
| A3 — essential obligations are conserved | SKILL.md "One owner per mutable fact" (a copy relocates the obligation, it does not delete it) |
...
| 10 — invariant semantics | idempotency (`domain-modeling.md`); retries, bounds, backpressure, interruption as an outcome (`events-and-consistency.md`); property-based tests of the algebra (`testing-and-governance.md`) |
