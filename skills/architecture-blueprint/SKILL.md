...
description: Apply the Modern Architecture Blueprint (2026), a vertical-slice-first method for designing long-lived software systems, derived from first principles. Covers modular monoliths, feature folders, type-driven domain modeling (newtypes, value objects, parse-don't-validate, illegal states unrepresentable), policies, domain events, consistency scopes, idempotency, functional core/imperative shell, typed authorization, fitness functions, and ADRs. Use when designing, scaffolding, structuring, refactoring, or reviewing the architecture of a backend or full-stack system; deciding folder or module layout; weighing repositories, CQRS, microservices, event sourcing, Clean/Hexagonal layering, or tactical DDD; modeling a business domain; deciding how modules communicate; or asking why an architectural rule holds. Trigger on "how should I structure this service", "where should this code live", or "is this over-engineered" even if architecture is never mentioned. Especially for TypeScript/Node, Rust, and Python.
---

# Modern Architecture Blueprint

Vertical-slice-first method for long-lived systems. Prefer business capabilities over technical layers, locality over abstraction, and strong types over runtime checks. This file is the decision layer; you MUST read the matching reference before applying a pattern in depth.

**Positioning.** This is NOT Clean/Hexagonal/Onion Architecture and NOT tactical DDD. The slice is the unit of behavior and the default home for logic. The domain exists, but as a **vocabulary, not a layer**: `domain/` holds the types slices share (newtypes, value objects, invariants), and nothing is routed *through* it. You MUST NOT introduce a domain layer, an application/use-case layer, or concentric dependency rings; entities, aggregates, and repositories appear ONLY on a named trigger (see Decision rules). From DDD this skill keeps the strategic half only — capabilities as bounded contexts, business language, explicit concepts.
...
**Derivation tags.** Every rule here is a consequence of `references/first-principles.md`. `(1, 8)` names the principles a rule derives from, `(frame)` the proof-obligation frame and its currency ladder (rungs 1–4), `(A1–A4)` its foundations, `(ledger)` the pattern-ledger entry carrying a pattern's conditions, `§Name` a section of that file (or of the reference named beside it). Challenge a rule through its derivation, never by importing a competing pattern.
...
- **SHOULD / SHOULD NOT** — strong default (a heuristic); deviate ONLY with a stated, demonstrated reason, and ONLY for an exception this skill explicitly names. You MUST NOT invent other exceptions. When unsure, follow the SHOULD.
...
- **Focused design question** → use Decision rules, then open the matching reference below.
...
| You are working on… | Read (`references/`) |
| --- | --- |
| Folder/module layout, module public APIs and READMEs, dependency direction, anti-corruption boundaries, composition root, capability dependencies and seams, state ownership and lifetimes | `structure-and-boundaries.md` |
| Naming a file or folder: role names (`handler`, `mapper`, `store`, `gateway`…), `utils`/`helpers`, names to avoid | `role-vocabulary.md` |
| Modeling a concept: newtypes, value objects, parse-don't-validate, typed config, illegal states, absence, exhaustiveness, lifecycle typestate, error taxonomy, policies, rich domain objects, functional core, temporal modeling, idempotency, persistence | `domain-modeling.md` |
| Domain events, event naming and versioning, direct call vs. event, consistency scopes, outbox, sagas, copies and caches, cancellation, bounded concurrency, retries and backpressure, scatter-gather, observability | `events-and-consistency.md` |
| Authorization, the Permit Pattern (`can`/`Permit`/`Denial`/grants), access control, permissions vs. roles, deny-by-default | `authorization.md` |
| Unit/integration/acceptance tests, property and contract tests, test placement, fitness functions, ADRs, AI context files | `testing-and-governance.md` |
| Justifying or challenging any rule; a situation no rule covers; two rules in tension; the pattern ledger; proof obligations, validity frames, consistency scopes, local checkability; the nine tests | `first-principles.md` |
...
The fourteen principles compress to three reductions and one direction of flow (§Compression). Applied to code:

**Reduce possibility** (1, 3, 4, 12)

- **Establish once, carry the receipt.** Parse at the boundary into types that prove their own validity, and enclose each resource's lifetime in its owner's; nothing downstream re-checks validity or liveness.
...
- **Compose, don't inherit.** You MUST NOT build business behavior through inheritance hierarchies (`BaseService → AbstractService → ConcreteService`): a subclass couples implicitly to the whole base behavior (5), and the chain must be read end-to-end before any method can be trusted (14). Compose small types, functions, traits/interfaces, generics, and enums. Extending a base class a framework requires is a technique, not a design.

**Reduce propagation** (8, 9, 10, 11, 13)

- **Capabilities at the top.** Within the application source tree (for example, `src/`), top-level folders MUST be business capabilities (`orders/`, `billing/`…). The ONLY sanctioned exceptions are optional `platform/` (business-free technical substrate) and `tests/` (cross-module tests); any other technical-concern folder there is a defect. (*Convention*; the requirement it implements is coupling by shared reason for change, 13.)
...
- **Authorization** (7, 6, 2, 1) — gate by capability; you MUST NOT use role conditionals (`if role == "admin"`). Authorization MUST follow the **Permit Pattern**: `action.can(actor) -> Permit<G>` — a pure decision over already-loaded context, evaluated at **use-case entry**, deny-by-default, returning a typed `Permit` (never a `bool`) whose `Denied` arm distinguishes `NotAllowed` from `NotPossible` and whose `Granted` arm carries the weakest useful artifact (`()`, a query `Filter`, or a domain grant). It lives **inside the slice** that owns the action, never in a module-wide policy grouping, and MUST NOT live on the entity: the entity guards business invariants, the action's `can` guards who-may-act. A permit is not a lock — the execution boundary still enforces its state assumptions atomically. Roles are capability groupings assigned at the edge. See `references/authorization.md`.
...
Names MUST speak the business language: `approveOrder()`, `reserveInventory()`, `calculateShippingCost()` (14 — understandability is load-bearing). You SHOULD NOT use vague verbs (`process`, `execute`, `run`, `handle`) unless intent is obvious from context. Events MUST be past-tense business facts (`OrderApproved`, `InventoryReserved`) and MUST NOT be vague mutations (`OrderUpdated`, `EntityChanged`) — *convention*, motivated by 1: a receipt that does not say what happened is not a receipt. `can` is reserved for the authorization protocol (`action.can(actor)`); business eligibility reads `isRefundable`, `isEligible`. A name is testimony (3): it informs the reader and enforces nothing, so a name never substitutes for a type.

File and folder names are a separate axis: they name the **role** a file plays (`handler`, `parser`, `mapper`, `store`, `gateway`), which is why a generic word is acceptable there and not in a business operation name. You MUST name a file for its actual responsibility, and SHOULD fall back to `utils` (business-free generics, in `platform/`) or slice-local `helpers/` ONLY when no specific role fits. See `references/role-vocabulary.md`.

## Avoid by default

Without a present, demonstrated need, each of these is a defect under the Core rule: microservices first, repositories everywhere, CQRS everywhere, event sourcing everywhere, layer-based folders, factory proliferation, specification proliferation, framework-centric architecture.
...
1. **Organization** (13, 14) — Top-level source folders are business capabilities, the ONLY exceptions being optional `platform/` and `tests/`? Features are vertical slices, with no technical-layer folders at any level? Code that changes together lives together? Each file's name states its actual role, with `utils`/`helpers` only where no specific role fits?
2. **Boundaries** (5, 6, 2) — Narrow module public API (`orders/api`)? No module reaching into another's internals, its tables and event internals included? Dependencies explicit and acyclic, with the composition root outside `platform/`? Slices receive narrow capabilities, not a database handle or god service? Foreign models stop at a gateway/translator? Authorization a `Permit`-returning `can` at use-case entry, in the slice (deny-by-default), kept OUT of entities, never a `bool` and never scattered role conditionals?
...
5. **Behavior** (6, 11) — Logic in slice + functional core by default? Behavior centralized on an object ONLY on a named trigger (flag rich objects that have not earned it), and no larger than its invariant demands? Shared `policies/` genuinely shared by 2+ slices and distinct from reactive processes? Any domain layer, use-case layer, or dependency ring that routes behavior through `domain/` (see Positioning)?
