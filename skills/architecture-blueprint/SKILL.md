...
- **Policy vs. specification** (6, 1, 5) — a **policy** is the default home for a business decision; it bundles related decisions/calculations (`refundPolicy.isRefundable(order)`, `refundPolicy.refundAmount(order)`; `can` is reserved for the authorization protocol). Use a **specification** (composable `isSatisfiedBy` predicate) ONLY when actually composing predicates or driving queries; otherwise a method/function (`customer.isEligible()`) SHOULD be preferred. The `policies/` folder SHOULD appear only when decision logic is shared by 2+ slices; authorization is NOT an exception — it lives in the slice that owns the action (see `references/authorization.md`). See `references/domain-modeling.md`.
...
| Authorization, the Permit Pattern (`can`/`Permit`/`Denial`/grants), access control, permissions vs. roles, deny-by-default | `references/authorization.md` |
...
5. **Behavior** (6, 11) — Logic in slice + functional core by default? Behavior centralized on an object ONLY on a named trigger (flag rich objects that have not earned it), and no larger than its invariant demands? Shared `policies/` genuinely shared by 2+ slices and distinct from reactive processes?
