...
| A stuck agent noticed | An agent's tool call can wait on the auto-mode classifier with no answer and no timeout; the lead, idle between turns, sees nothing (units-docs, 2026-09-25, 2.1.281: four of sixteen agents, one hour and 45 minutes). `next` names an agent whose transcript has been quiet `stall_minutes` (policy: 15) and says to stop it and dispatch a fresh one; `gauntletctl wait`, started in the background before a turn ends, wakes the lead then | The `/goal` check-in, about an hour later |
...
| Agent transcripts | a subagent's transcript is `<lead transcript without .jsonl>/subagents/agent-<agent_id>.jsonl`; the start hook's `transcript_path` is the lead's (2.1.281). An agent the lead stopped with `TaskStop` ends it with a line containing `[Request interrupted by user` Under the run's sandbox a sandboxed controller cannot read them (units-3, 2026-09-26), so the hooks stamp `.gauntlet/private/activity/<agent_id>` on every call |
