...
| Agent transcripts | a subagent's transcript is `<lead transcript without .jsonl>/subagents/agent-<agent_id>.jsonl`; the start hook's `transcript_path` is the lead's (2.1.281). An agent the lead stopped with `TaskStop` ends it with a line containing `[Request interrupted by user` |
