...
Pair preparation: the reference half is prepared once, at round zero, from the frozen renders: PNGs at the named viewports and scale factor, state variants, and for motion a scripted scroll and hover recorded to a clip; when the reference is source, `freeze-verify` rendered it, under isolation, and nothing renders it again. Every round the LEAD renders ours only, with the same Playwright script - same viewport, scale factor and crop, browser chrome removed, the same scripted scroll and hover, clips of equal length - into the artifact path, and `pair` copies it beside the reference half under seeded labels; the confirming CRITIC gets the same files with the labels inverted. For games, same resolution, matching framing and lighting hour, same frame count. BUILDER screenshots never reach a CRITIC.
