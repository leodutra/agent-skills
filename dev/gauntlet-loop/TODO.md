...
- [x] **C1. Cheap spikes.** Narrowed 2026-09-24. S1 declined: no enforced rule depends on frontmatter hooks, since every hook is registered in settings (D16), so its answer changes nothing. S3 was already observed (a capped agent fires no `SubagentStop`; harness evidence). What is left is plan 20.3, one write-mode case with `SKILL.md` alone: `dev/gauntlet-loop/eval/run.sh --skill-only --only <case>`, a few cents. Run 2026-09-24 on `writing-basic`: pass, $0.06 on claude-sonnet-5, `SKILL.md` alone.
- [x] **C2. A real run, the way a user runs it.** Done: bytesize-2 exposed F9; bytesize-3 won (see Stage D). I prepare `~/Work/bytesize-2` from the bytesize skeleton, install the fixed skill, commit, check `detect` says tier 2, and write the paste prompt from `SKILL.md` (same goal, 40 invocations, 3 hours). The user opens `claude` there, accepts trust, sets `/effort xhigh` and auto mode, optionally `/sandbox` strict (which enables `freeze-verify`, lets critics execute, and covers AC-15.4 and S14), and pastes it. Cost: the user's usage, at most 40 subagent spawns; tens of dollars. It must show for the first time on v2: verdicts attested through the handback, a builder's green suite line, no false boundary blocks, hooks on the lead, the `/goal` evaluator reading the status line, a swapped confirmation, a wave, the whole gate over two pieces, and a routed loss with a resumed builder if a loss occurs.
...
- [x] **D1. Findings.** Done after bytesize-3, units and units-2 (F13 to F36). Read the log, the report and the transcripts; each finding test-first, as in Stage A.
...
- [x] **E1. Every controller command ends with what is next.** A command that moves the run prints the `next` lines after its own output, so the lead stops calling `next` separately (6 calls in bytesize-3, 17 in units, each re-reading the whole context; first counted as 12 and 30 with `status` mixed in). `next` stays for a resume. The runbook's "after every event run `next`" becomes "read the next lines each command prints".
- [x] **E2. Load less by default.** `example-run.md` is read only when the lead is stuck, never at round zero (every fresh session counted as "a first run", and it is 18 KB carried for a hundred calls). The harness file keeps what a run needs (the install, the needs-and-fallback table, the settings the controller reads); the dated spike and check rows move to `dev/gauntlet-loop/design/harness-evidence.md`, where humans read them. The runbook says the controller's and the hooks' source are never read in a run: `next`, `--help` and a denial's reason are the interface (the units lead read 28 KB of controller source; F20 folds in here). Sizes stay gated by `sizes.sh`.
- [x] **E3. One command per mechanical round.** `gauntletctl round <piece>`: floors, then the pair if they are green, printing the reader's path or the red floor; the confirmation stays `swap`. Two lead calls per round become one, and the steps keep their order.
...
- [x] **E5b. The lead at `high`, measured.** `~/Work/units-3` is units-2 again with the pins installed: same skeleton, bars, envelope and prompt. The user runs it with `/effort high`. Compare the lead's rows with units-2 (calls, tokens, cache reads, output, wall) and quality (rounds, flips, verdicts, gaps). One run is a signal, not proof. E6 waits until it ends: the runbook is global, so changing it before would put two changes in one run. **Run 2026-09-26: win, inconclusive** (lead tokens 292 k against 164 k, confounded by the lead reading the controller's source before `init`); D36 stays `xhigh`. Details in `acceptance.md`.
  - Prepared 2026-09-24: `~/Work/units-3`, units-2's skeleton (`1356920`) and the skill as of `2b9953d` (`6d1cc55`), updated 2026-09-25 to `20e1877` (`023aa50`) before any run, so it has the F39 to F43 fixes; `detect` says Tier 2, and the three pins are installed. Besides the lead's effort it differs from units-2 in those fixes, which touch critics' blocks more than the lead. The prompt is C3's, unchanged. Waiting on the user: open `claude` there, accept trust, `/effort high` (the one difference from units-2), auto mode, `/sandbox` strict (units-2 had it on), paste. When it ends, add a units-3 column to the baseline table in `acceptance.md`.
...
- [x] **W1. A writing-domain run.** `~/Work/units-docs`: units-2's winning library (`gauntlet/wave-1`), no README, the skill installed. The goal is its README, judged per piece against the bytes.js 3.1.2 and ms 2.1.3 readmes; 40 invocations, 3 hours, like the first run. It checks the writing path: a writer instead of a builder, reading floors, a single-document artifact, prose critics. Its numbers are the writing baseline; nothing to compare them with yet. **Done 2026-09-25: win**, 16 of 40 invocations, 2.3 of 3 hours; results and F39 to F43 in `acceptance.md`.
...
- [x] **F39 to F43, from units-docs.** Critic hook judged raw text (28 false blocks); `pair` cut prose lines; four stuck agents unnoticed for 1 h 45 min; the wave merged into `main`; an ended run's clock kept running. Fixed test-first (A62 to A66); the 28 blocks replayed and pass.
- [ ] **The harness's silent classifier.** Three tool calls waited 12 to 53 minutes on the auto-mode check with no verdict, where the docs promise a timeout and a denial. It is Claude Code's, not the skill's: the user can report it with `/feedback` (session `78e12932` in `~/.claude/projects/-home-leo-Work-units-docs/`, 2026-09-25 03:09 to 05:10 UTC, 2.1.281). D41 limits the damage to about 15 minutes.

- [x] **F44 to F47, from units-3.** The lead read the tooling's source (now denied during a run); the stall detector was blind under the sandbox (hooks now stamp activity); required tests in pairs; bare-name comment stripping. A67 to A70.
