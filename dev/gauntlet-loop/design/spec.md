...
- **FR-1.3** A `PreToolUse` hook, registered in the target repository's settings and acting only when `agent_type` is `reader` or `reader-alt` (A9), binds a reader to the pair it opens first, rewrites its shell commands to start inside that pair, and denies any `Read`, `Glob` or `Grep` whose path resolves outside that pair (`.gauntlet/pairs/<piece>/`), and any `Bash` command that names a path outside it or names `state.json`, `events.jsonl`, `.gauntlet/private`, `reference/`, `heldout/`, the workbench or `CLAUDE.md`. Every block is logged (`BLIND_BLOCK` event with agent id and path) and the denial reason says only "outside your working set".
- **FR-1.4** State that would break blindness lives under `.gauntlet/`, outside every worktree and every pair: the state file and event log at `.gauntlet/state.json` and `.gauntlet/events.jsonl`, the reference manifest, A/B mapping and run secret under `.gauntlet/private/`, verdicts under `.gauntlet/verdicts/`. Every tool, the lead's included, is denied `.gauntlet/private/**` by the `controller-only` hook, and the lead reaches the run's state through the controller; no permission rule names these paths, since the harness would turn it into a sandbox mask the controller cannot read through (A23).
...
- **FR-1.7** Readers set `omitClaudeMd: true`, so the repository's `CLAUDE.md` does not reach a critic (A5). `freeze`, the first command that knows the reference's name, checks the repository's `CLAUDE.md` files for that name and the loop vocabulary, and lowers the claimed tier to 3 only when the installed reader definitions lack the field.
...
- **FR-3.2** A settings-level hook acting on `editor` and `editor-fast` (A9) binds a builder to the worktree it first writes in, denies it reads of `heldout/`, `bench/` and of `.gauntlet/` outside that worktree (A4), and denies edits to migrations, infrastructure and deploy configuration, secret and credential files, `.claude/`, `.gauntlet/`, and any path outside the piece's worktree; and denies shell commands matching deploy, publish, push, migrate and destructive patterns. Patterns are listed in the policy file. The denial message names the rule and says to return `BLOCKED: <reason>` instead of retrying.
...
- **FR-6.16** The write-mode line under the prompt says `/effort xhigh`. Since A59 that sets the lead's effort alone: per-role effort is pinned in the agent definitions (`editor`, `reader` and `reader-alt` at `xhigh`; `editor-fast` and `author` at `high`), since the harness supports it (§14.1), and a pin comes out under README D36's condition.
...
- **FR-9.1** The README credits Matt Shumer for the method and names no other implementation, repository or link; the alternatives this skill decided against are listed unnamed, each with the entry that argues it. C1 was settled this way by the user on 2026-09-19.
...
gauntlet-loop/
  ...
  install/claude_code.py           copies the package into a target repository and merges its settings (A8)
  eval/                            write-mode cases, critic pairs, check script, runner, baseline
  tests/                           controller tests
```

Installed layout in a target repository (A8, P1): `.claude/hooks/gauntlet/` holds the controller, the hook scripts, `policy/` and `MANIFEST.sha256`; `.claude/agents/` holds the definitions; the hooks block and the allowlist are merged into `.claude/settings.json`.
...
.gauntlet/
  ...
  wt/<piece>/                      one git worktree per piece (A8, P2); never committed
  staging/                         files an author hands to `floor add --from` (A3, A8); never committed
  .gitignore                       one line, `*`; `commit` stages the committed set with `git add -f` after the scan (A8)
  state.json                       source of truth; never committed; tool reads denied (controller only)
  private/                         secret, mapping/, bindings/, archived pairs, manifest; never committed; tool reads denied
...
| `piece open|split` | judgment recorded | Opens pieces with parent, referent, mode, allocation; `PIECE_OPENED`. Referent and mode are the judgment events `REFERENT_SELECTED` and `MODE_SELECTED`, recorded through `event` before the piece opens, or the `--referent` and `--champion-challenger` flags of `open`; both are rejected once the piece is open (A13) |
...
| `pair <piece> [--prepare]` | mechanical | Reference half once; ours per round; labels from seed; strip what is not the deliverable; refuse on secrets and on a loop trace inside it (A13); emit prompt |
...
Champion-challenger rows (A1): `AWAITING_CONFIRMATION` + `CHAMPION_REPLACED` (derived from `CONFIRMATION_WIN` in this mode) → `ACTIVE`, the champion replaced and the loss counter reset; `CRITIC_LOSS` counts one challenger loss; after two in a row a gap-only verdict (shape: `PAIR`, `EVIDENCE`, then `GAP: <observation>` or `GAP: none`, no `WINNER`) is dispatched and yields `GAP_ONLY_RESULT`: a named gap from either reader → `ACTIVE` with that gap routed; `GAP: none` from `reader` → `AWAITING_CONFIRMATION`; a second `GAP: none`, from `reader-alt` on the swapped pair, derives `CONVERGED` → `CONFIRMED` with `mode: champion-challenger`. `CONVERGED` is the single exception to "no `CONFIRMED` without `CONFIRMATION_WIN`" and carries the same two-reader, two-model requirement; it is rejected outside this mode and without both verdicts, `reader` first. Other rows the draft omitted (A10): `AWAITING_CONFIRMATION` + `VERDICT_INVALID` stays put (a discarded confirming verdict does not drop the win; a confirmation attested from `reader` is discarded this way); `CONFIRMED` or `INTEGRATED` + `FLOOR_FAIL` → `ACTIVE` (a merge, or an amended floor, went red); `INTEGRATED` + `CRITIC_LOSS` derives `RECHECK_LOSS`; a split marks the parent superseded and every later event on it is rejected.
...
**Origin authentication is tier-dependent, and the ledger says which applies.** At Tier 1 the hook command carries an attestation key from private storage that the sandbox's read deny keeps from every tool; a payload without the key is refused outright, so the harness is the authenticated source. At Tier 2 the key is only string-match protected, so `attest` enforces provenance structure but does not authenticate origin. A forged payload for a live agent id then collides with the genuine `SubagentStop` when it fires. It does not always fire: spike S3 showed no `SubagentStop` when `maxTurns` is reached, so the guarantee covers readers that stop, a capped reader leaves an open attempt for the lease to close, and the report lists every registered reader that never stopped (A6). When it does fire: the controller records `ATTEST_CONFLICT`, voids both attestations and every transition derived from them, returns the piece to `ACTIVE`, and lists the conflict first in the report. `CONFIRMED` therefore means, at Tier 1, that a qualifying reader execution produced the result, and at Tier 2, that no unresolved conflict stands against it. Neither is called stronger than that.

### 8.5 Enforcement

| Hook | Where | Event and matcher | Denies | For whom |
| --- | --- | --- | --- | --- |
| `critic-blind` | target repo `.claude/settings.json` (A9) | `PreToolUse` on `Read|Glob|Grep|Bash` | paths outside the reader's bound pair; commands naming private state, reference, held-out, workbench, `CLAUDE.md` | readers only |
| `builder-boundary` | target repo `.claude/settings.json` (A9) | `PreToolUse` on `Read|Glob|Grep|Edit|Write|Bash` (A4) | policy path and command patterns; anything outside the worktree | builders and smoother |
...
Agent definitions: `tools:` as in FR-1.1; no `SendMessage` in any role (so the sibling roster is never shown); the `reader` and `reader-alt` bodies are the critic protocol (FR-1.8); `model:` pinned in `reader` and `reader-alt` to the two families the policy names (FR-17.13), unset for `editor` (the lead passes it per call), `sonnet`-class for `editor-fast`; `effort:` `xhigh` for `reader`, `reader-alt` and `editor`, `high` for `editor-fast` and `author` (A59); `maxTurns:` set on every role as a runaway guard, well above the most turns the role has used (A60), since a capped agent fires no `SubagentStop` (S3) and its attempt stays open until the lease closes it; no `memory:`; no `skills:`.

Allowlist (`permissions/allowlist.json`): allow `git`, the domain's build, test and render commands, `gauntletctl`; deny `Read`/`Edit` on `.gauntlet/private/**`, `.gauntlet/state.json`, `.gauntlet/events.jsonl`, secrets and env files; `permissions.blockReadsOutsideWorkingDirectories: true`; a sandbox block with `allowUnsandboxedCommands: false`, `network.allowedDomains` limited to registries, and one read deny, the attestation key. `credentials` deny entries are recommended for user or managed settings, since the harness ignores them in project settings. The sandbox rule (A7, settles C5 and C11): no `excludedCommands` ship, because a compound command rides out of the sandbox on an excluded prefix (S14); `freeze-verify`, `floors` and `rerun` execute code the run does not trust and always run sandboxed; private storage sits behind the permission denies and `controller-only`, not a sandbox deny, so the sandboxed controller reads its own secret; the attestation key alone is sandbox-denied, readable only by hook processes (S4), which is the whole of Tier 1 origin authentication. `detect` reports tier 2 or 3; Tier 1 blindness needs a container per critic, which the controller cannot see and therefore never claims.
...
- **AC-9.1** README attribution names Matt Shumer and nobody else; the alternatives decided against are listed unnamed (C1, settled 2026-09-19).
...
**C1. Attribution.** Settled by the user on 2026-09-19: the repository credits Matt Shumer for the method and names no other implementation, repository or link, as the user first asked on 2026-08-26. The alternatives this skill decided against stay in the README, unnamed.
...
Holes found after the draft. Each row names the phase whose commit changed the text above; A1 to A8 are described in `plan.md` §1.4.

| Id | Date | Change | Phase |
| --- | --- | --- | --- |
| A5 | 2026-09-18 | FR-1.7: readers set `omitClaudeMd`; the `CLAUDE.md` check moves from `init` to `freeze`, which writes the warning into the manifest | 1 (definitions), 7 (check) |
| A6 | 2026-09-18 | FR-17.12, §8.4.6: single use and collision voiding apply to readers. An editor keeps its agent id across rounds (S12), so a stop closes the open builder attempt (`PIECE_OPENED` and every `GAP_ROUTED` open one) and a stop with no open attempt is a note. The payload field is `agent_transcript_path`; the verdict comes from `last_assistant_message` (S11). A conflict voids everything the piece did from the forged attestation onward, not only the two attestations, since later moves rest on it. `maxTurns` does not guarantee a stop (S3) | 9e |
| A7 | 2026-09-18 | §8.5, C5, C11: the sandbox rule above replaces "exclude `gauntletctl` from the sandbox" | 7 |
| A8 | 2026-09-18 | §8.1: `.gauntlet/.gitignore`, `wt/`, `staging/`, `install/claude_code.py`, the installed layout | 1 |
| A1 | 2026-09-18 | §8.4.2: champion-challenger has a path through the machine: `CHAMPION_REPLACED`, the loss counter, the gap-only verdict shape, `GAP_ONLY_RESULT`, and `CONVERGED` in its confirmed, two-reader form (C13) | 9b |
| A2 | 2026-09-18 | §8.4.1 `pair`, FR-6.3: no N-way pair ships. Each variant is an ordinary pair against the reference and a variant's win is confirmed like any win; when every variant lost, one `pair --pick` in the champion-challenger layout (anchor plus two of ours) says which one continues the piece, and moves no state | 9c |
| A3 | 2026-09-18 | FR-3.3, FR-5.2, §8.4.1 `floor`: floors can grow after round zero. `floor add\|amend <id> --cmd <command> --from <staged file> --to <protected path>` copies a file an author staged under `.gauntlet/staging/` into the protected tree itself; `protect_floors` exempts the `author` until `PLAN_COMMITTED` and nobody after it | 3 (interim), 9d |
| A4 | 2026-09-18 | FR-3.2, §8.5: `builder_boundary` also matches `Read\|Glob\|Grep` and denies held-out material and run state outside the agent's own worktree | 3 |
| A9 | 2026-09-18 | FR-1.3, §8.5: the deny hooks are registered in settings and branch on `agent_type`, not declared in agent frontmatter. Spike S1 found frontmatter hooks skipped, with a debug-log error only, in a folder whose trust dialog was never accepted; settings-level hooks fire there and carry `agent_type` (S10). A reader is bound to the pair it opens first and its shell is rewritten to start there (S6 settles C3) | 1 |
| A10 | 2026-09-18 | §8.4.2, §8.4.6: rows and names the draft omitted: `AWAITING_CONFIRMATION` + `VERDICT_INVALID`, `FLOOR_FAIL` after confirmation, the superseded parent, `BLOCKER_CLEARED` as a judgment event, and the fact events `FLOOR_ADDED`, `FLOOR_AMENDED`, `GAP_ONLY_RESULT`, `CHAMPION_REPLACED`, `CONVERGED`, `RECHECK_LOSS`, `LEAD_TURN`, `RUN_ENDED` | 9b to 9g |
| A11 | 2026-09-18 | §6 Size, FR-8.3, AC-NN.10: the template plus three full examples is 5.5 KB of `SKILL.md`'s 7 KB, so the examples are slot fills of the template and `eval/wordcount.py` assembles each one and counts it. Write mode no longer reads a domain file by default; bar shapes moved to `references/what-breaks.md`. AC-7.1: `SKILL.md` keeps exactly two harness tokens, `/goal` and `/effort xhigh`, because the paste prompt and the line under it are harness text by design (AC-8.2, AC-6.3) | 11, 12 |
| A12 | 2026-09-19 | §8.1: `eval/`, `tests/` and the design documents are not part of the package. They live in `dev/gauntlet-loop/` at the repository root, so the mirrored skill carries only what a session or a run uses | after release |
| A13 | 2026-09-19 | §8.4.1 `piece`: `piece referent` and `piece mode` never shipped and are not needed. The judgment is recorded by `event REFERENT_SELECTED` and `event MODE_SELECTED` before the piece opens (or by `piece open --referent` and `--champion-challenger`), `MODE_SELECTED` takes `reference` or `champion-challenger` only, and both are rejected for an open piece: a piece's bar does not move under its verdicts. FR-6.9, FR-NN.8, plan 9c: `pair` strips what is not the deliverable (git history, harness files, dependencies, build output, run state) and refuses a loop trace inside it, a line or a filename; a rename or a cut in the copy would change what is judged and leave the trace in what ships (README D40). §8.4.5: the policy also carries the path lists of `critic_blind`, `author_scope` and `controller_only`, so no hook holds one; `next` names the shared-by-rule globs and the offload sizes. §8.5: `author_scope` also matches `Bash` and holds an author's shell writes to its trees; the shared shell matcher counts a bare word as a path when it exists, takes a redirect or follows a plain write verb | gaps a to e |
| A14 | 2026-09-19 | FR-6.5, FR-6.8 (floor outputs in a pair): both sides or neither. Ours were copied into the pair whether or not the reference carried its own, under the lead's floor ids and with `$GAUNTLET_OURS` on the recorded command line, so a critic could read which side is ours. Now ours are attached only in a plain reference pair whose reference half carries verified outputs; both sets are numbered alike, and every path or variable that names a side is made neutral. `pair --prepare` finds `.verify` at the frozen root when the matched part is a subfolder. Found by reading, before the first real run | after release |
| A15 | 2026-09-23 | FR-12.2: the final `commit` also carries the floors (`tests/required/`, `heldout/`, `bench/`), after the secret scan, so the pull request carries the tests that judged it; `commit --plan` still carries none, and the reference's bytes are never committed. The first real run's pull request would have had no tests (decided by the user, 2026-09-23) | after release |
| A16 | 2026-09-23 | FR-17.12, §8.4.6, A6: an agent's return is the `message` of its last handback tool call when it made one, read from `agent_transcript_path`; otherwise `last_assistant_message`. On 2.1.277 builders handed back through the tool and the last message held only chatter, so a `BLOCKED:` return was filed as a build (spike S11 overturned) | after release |
| A17 | 2026-09-23 | FR-3.2, §8.5: the hooks' shell matcher reads redirects on shell text only, with quoted strings and here-document bodies removed, and a revision path (`HEAD:heldout/x`) counts as the path it names. A builder's quoted `node -e` script with a `>` in it was denied as a write outside its worktree | after release |
| A18 | 2026-09-23 | FR-15.7: `pair` adds a controller line to `PROMPT.md` when the piece's referent was never verified under isolation: judge by reading, execute nothing. The rule forbade executing reference code without isolation, but nothing told the critic, who cannot tell which side is the reference | after release |
| A19 | 2026-09-23 | FR-6.2: floor files find ours at `$GAUNTLET_OURS`, else the working directory, so a builder runs its required suite from its worktree with no env prefix. Auto mode refused the env-prefixed command, so no builder could run its own suite | after release |
| A20 | 2026-09-23 | §8.4.1 `status`: `--peek` prints the status line and records nothing. Every plain `status` records a lead turn, expires leases and can end an overdue run, so a human checking a run changed it (decided by the user, 2026-09-23) | after release |
| A21 | 2026-09-23 | FR-13.1: lead tokens per confirmed piece. The start hook reads the lead's transcript, which every `SubagentStart` payload names (S2), and records the count on `AGENT_REGISTERED`, so `metrics` still computes from the log alone | after release |
| A22 | 2026-09-23 | FR-6.13, FR-6.14, §8.4.5: the freeze is the lead's own command at any size, since `gauntletctl freeze` returns only the manifest; the freezer specialist and `offload.reference_files` are gone, and the author offload applies to floors only. The lead cannot count a reference's files before freezing it (decided by the user, 2026-09-23) | after release |
| A23 | 2026-09-23 | FR-1.4, §8.5 allowlist: no permission rule names `.gauntlet/`. The harness merges `Read` deny and `Edit` rules into the sandbox, so with the sandbox on the shipped denies hid `events.jsonl`, `state.json` and `private/` from the controller, which runs in the lead's sandboxed shell, and no run could start (the bytesize-2 run). `hooks/controller_only.py` now also matches `Read\|Glob\|Grep` and denies every agent and the lead `.gauntlet/private/` (by path for file tools, a string match for the shell). The attestation key is made by the first `SubagentStart` hook, which runs outside the sandbox, never by the sandboxed `init` | after release |
| A24 | 2026-09-23 | §8.4.1, FR-6b.3: a controller that cannot read or write a file refuses (exit 3) and names the path, never a traceback; an installed controller that errors stops the run and is reported, never continued by hand. The by-hand path is for a run with no controller at all. In bytesize-2 the lead met a traceback and went on by hand at Tier 3, where no pair can be built | after release |
| A25 | 2026-09-23 | §8.5 `controller-only`: a denied compound command that names the controller says to run it alone on its line and each write as its own command. The matcher stays coarse and fails closed | after release |
| A26 | 2026-09-23 | FR-3.3, §8.5 `protect-floors`: the controller alone on its line passes, as it does `controller-only`; one helper decides it for both. `freeze-verify reference/<name> --cmd "npm install"` was denied in the bytesize-3 run, because the hook matched the write verb inside the quoted argument and the reference path beside it, so no run could verify a reference that needs an install step | after release |
| A27 | 2026-09-23 | FR-3.2, §8.5, A17: a rule on where writes land (`builder-boundary`'s worktree, `author_scope`'s trees) checks what a command writes: redirect targets and the operands of write verbs (the last one of a copy or a move), a `sed -i` file, `dd of=`, a git path. A target named through a variable is denied. The bytesize-3 author was denied six commands whose `sed` scripts counted as paths outside its trees | after release |
| A28 | 2026-09-23 | FR-1.3: a reader is bound to the pair its first file tool or first shell command names, when every path that command names lies in that one pair. Both bytesize-3 readers lost a turn to `cat <pair>/PAIR_ID` denied as unbound | after release |
| A29 | 2026-09-23 | FR-6.5, FR-1.x: "names stripped" is now done, by `pair`, on both sides alike: identity files (README, LICENSE, CHANGELOG, HISTORY, AUTHORS, NOTICE) leave a side of more than one file, `package.json` loses its identity fields, and comment-only lines that carry a copyright or the reference's name go; code is never changed. Before this the bytesize-3 lead had an author anonymise the reference by hand, and the first critic still named it from its code (README D8's residual risk) | after release |
| A30 | 2026-09-23 | §8.5 agent definitions: a builder returns its directory for code, and a file only for a single document. The bytesize-3 builder returned `src/index.js`, the floors got a file path, and the lead amended all four floors with a directory walk | after release |
| A31 | 2026-09-23 | FR-13.1: tokens are counted once per message (a message's usage repeats on each of its transcript lines) and leave out cache reads, which re-read context already counted. The bytesize-3 report put its lead at 23.8 million tokens for a run of under an hour; recounted, about 270 thousand | after release |
| A32 | 2026-09-24 | §8.5, A26: "the controller alone on its line" reads operators on shell text only, so a `;`, `|` or `>` inside a quoted `--cmd` argument does not chain it; a `$(...)` or backtick inside double quotes still does, since the shell runs it. The units run's `freeze-verify --cmd '... ; echo "exit $?"'`, alone on its line, was denied by `controller-only` | after release |
| A33 | 2026-09-24 | FR-15.1, FR-3.3: `freeze` writes only under `reference/`, the tree the floor hooks protect. The units lead froze a combined copy into `reference-whole/`, where no hook guarded it | after release |
| A34 | 2026-09-24 | §8.4.6, FR-5.5: a thirteenth judgment event, `QUESTION_FILED` (note, piece optional), adds a question to the open questions the workbench and now the report show; it moves nothing. The spec, the prompt and the runbook sent questions and unattended DERIVED items there, but only a blocked piece could fill the list, so the units lead wrote its five open decisions to a staging file | after release |
| A35 | 2026-09-24 | FR-15.7, §8.5: `freeze-verify`'s probe counts a write outside the project as an escape only when it lands on the project's own filesystem. The harness's sandbox lays a throwaway tmpfs over the home directory and binds the project into it, so a write to the parent succeeded and vanished, and the probe called two isolated runs (bytesize-3, units) not isolated: their references were never verified and their critics could only read | after release |
| A36 | 2026-09-24 | FR-3.2, §8.5: builders and authors may write the harness's session scratchpad, where the harness sends every agent for temporary files; a builder is never bound by a scratch write; the "outside your directory" denial says to keep files inside the worktree and to return `BLOCKED` only when the work itself needs something outside. The units run's bytes builder was denied a `check.mjs` in the scratchpad, returned `BLOCKED` as the denial told it to, and parked with its required suite green | after release |
| A37 | 2026-09-24 | FR-14.6, FR-4.3: "nothing left" (every remaining piece parked) ends the run whichever event made it so, not only a park; the whole gate is offered only when every piece is integrated. In the units run bytes parked while durations was active; when durations finished, nothing ended the run, and `next` offered a whole gate over a parked piece | after release |
| A38 | 2026-09-24 | §8.5 `controller-only`: `git add` is denied with `-f`/`--force`, or when a path it names resolves into `.gauntlet/` outside `wt/`; staging inside a piece's worktree is allowed. The rule denied any `git add` whose command mentioned `.gauntlet`, so the units lead could not stage the bytes builder's work in its worktree for the merge. Plain staging of run artifacts was never possible, since `.gauntlet/.gitignore` ignores everything | after release |
| A39 | 2026-09-24 | FR-13.1, A31: a message's usage is taken from its last transcript line. In a subagent's transcript a message streams over several lines and its output count grows (7, then 1,218), so A31's first-line count put one author at 919 output tokens instead of 17,863 | after release |
| A40 | 2026-09-24 | A29: identity files match in any case. `ms`'s lowercase `license.md` passed a case-sensitive list in the units whole gate | after release |
| A41 | 2026-09-24 | FR-6.17, A29: a pair never carries a top-level `reference/`, `heldout/` or `bench/`, on either side. The units whole gate's side of ours held the empty `reference/` folders the plan commit left behind, which said which side was ours | after release |
| A42 | 2026-09-24 | §8.1, FR-17.1: the controller and the hooks map a path inside `.gauntlet/wt/<piece>/` to the project. A worktree holds the plan commit's copy of `.gauntlet/`, and the units lead, running the controller there, wrote to that stale copy | after release |
| A43 | 2026-09-24 | §8.5 (ponytail audit, D5): every hook's write rule reads one matcher, `write_targets`: redirect targets, write-verb operands (`cp -t` and `--target-directory` included), `sed -i` files, `dd of=`, git paths, and what a `$(...)` or backtick writes. A target named through a variable, or a write verb that heads no command (`sudo rm`, `A=1 rm`, `xargs rm`, `find -delete`), holds every path the command names to the rule. The old verb-anywhere matcher is gone; `controller-only` and `protect-floors` no longer deny a harmless command for a word in it (`echo heldout > note.txt`) | after release |
| A44 | 2026-09-24 | FR-17.4, §8.4.1 (Stage E1): a command that moves the run ends by printing the next lines on stderr, the same lines `next` prints; `status` and the hooks' commands do not. The lead stops calling `next` after every event (6 such calls in bytesize-3, 17 in units, each re-reading its whole context); `next` stays for a resume. The command only prints: the lead still decides and dispatches | after release |
| A45 | 2026-09-24 | §8.4.1 (Stage E3): `round <piece>` runs the floors and, only when they are green, the ordinary pair, in one call; a red floor ends the round with no pair. It builds no pair where champion-challenger says none is due (a new champion stands unjudged, or the gap-only pair is next); gap-only, pick and recheck pairs stay with `pair`, the confirmation with `swap`. `next` names `round` at the floors step | after release |
| A46 | 2026-09-24 | §8.1 `install/claude_code.py` (D5): no `--verify`; `gauntletctl detect` is the one check of the installed files against the manifest, as the harness file's install steps already said | after release |
| A47 | 2026-09-24 | §8.5 `register` (D5): no separate script; `SubagentStart` runs `attest.py --start`, `SubagentStop` runs `attest.py` | after release |
| A48 | 2026-09-24 | §8.4.1 `workbench` (D5): removed; every transaction already rewrites `.gauntlet/workbench.md` | after release |
| A49 | 2026-09-24 | §8.4.5 (D5): the policy drops `envelope.write_mode_default` and `envelope.bootstrap`, which nothing read; `SKILL.md` carries write mode's 150 and 24, and the README marks the proportions as bootstrap values | after release |
| A50 | 2026-09-24 | §8.5 allowlist (D5): drops `user_settings_recommended`, which nothing read; the harness file says where credential denies go | after release |
| A51 | 2026-09-24 | FR-6b.4, FR-6.19, FR-7.1 (Stage E2): the lead loads less. `example-run.md` is read only when the lead is stuck; the harness file keeps the facts a run acts on, and its dated spikes and checks by hand move to `dev/gauntlet-loop/design/harness-evidence.md` (19.8 KB became 8.2 KB); the runbook says the lead never reads the controller's or the hooks' source (the units lead read 28 KB of it), which also closes F20 | after release |
| A52 | 2026-09-24 | FR-6.4 (Stage E4): commands and dispatches for independent pieces go in one turn, as parallel tool calls: one model call instead of one per piece | after release |
| A53 | 2026-09-24 | §8.5 (F31): a hook judges a variable where the shell would put it: one the command sets (`S=/x; cat $S/a`), a loop's words (one variant per word), `$TMPDIR` and `$HOME`; a `cd` partway through a chain moves where the next commands write; a quoted "-- /path" is split at its spaces. Only a variable it cannot know is judged unresolved. Nine of units-2's 18 blocks were such variables | after release |
| A54 | 2026-09-24 | FR-3.2, A36 (F32): the harness's whole temp dir (`/tmp/claude-<uid>`, `$TMPDIR` and the scratchpad under it) is scratch for builders, authors and the lead, never a path inside the project. The harness sends agents to `$TMPDIR`; A36 allowed only its scratchpad | after release |
| A55 | 2026-09-24 | FR-1.8 (F33): the reader definitions say to name the pair's full path in the first command (the shell starts there only after that) and to keep a scratch script in the pair, never in a temp folder. "Your shell already starts there" sent readers' first commands to the project root | after release |
| A56 | 2026-09-24 | `domains/coding.md` (F34): the critic runs what the pair holds and reads the floor outputs under `FLOORS/`; the held-out and hostile scripts are the lead's and never in the pair. "Runs the hostile-input script" sent five units-2 readers looking outside their pair | after release |
| A57 | 2026-09-24 | FR-15.7, A18 (F35): a referent counts as verified when it sits inside a verified copy or when every frozen copy under it is verified, so the whole gate over per-piece references is not told to judge by reading once they are all verified | after release |
| A58 | 2026-09-24 | FR-6.5, A29 (F36): a pair's `package.json` keeps only what loads it (`type`, `main`, `module`, `exports`, `imports`, `bin`, the dependency lists, `engines`). Blanking named fields left `version` 3.1.2, `keywords` and `devDependencies`, and every units-2 critic said which side was the published one | after release |
| A59 | 2026-09-24 | FR-6.16, §8.5 agent definitions (Stage E5): builders and critics pin `effort: xhigh` instead of inheriting the session's. A forgotten `/effort` shortchanged them, and Opus 5.5, the confirming critic's model, defaults to `medium`. The pins are for today's models: README D36 names the condition that removes each | after release |
| A60 | 2026-09-24 | FR-13.1, §8.5 `attest`, the agent definitions (F37 and the `maxTurns` review): an author's stop records `AUTHOR_DONE`, since units and units-2 recorded nothing and `metrics` left out every author's tokens and minutes. Every stop event carries the agent's turns, and `metrics` reports the most turns per role and the agents that never stopped. The definitions' old reason for `maxTurns`, that it makes `SubagentStop` always fire, was the opposite of S3; the caps are runaway guards, kept after measuring four runs (most used: editor 15 of 300, reader 14 of 40, reader-alt 11 of 40, author 27 of 80, editor-fast 15 of 80) | after release |
| A61 | 2026-09-24 | FR-6.5, A29 (F38): identity files still leave a side of more than one file, except one named like the file being judged. When the goal is a README, the README is the work: with the reference passed as its folder, `pair` deleted it from that side, and the critic had one document to compare. Found while preparing the writing-domain run | after release |
| A62 | 2026-09-25 | §8.5 `critic_blind` (F39): a reader's shell command is judged by what it opens and writes, by the shell's rules (`read_targets`, `write_targets`), not by every token: grep, sed, awk and jq programs, echo and printf text, and quoted heredoc data are not paths, a substitution is judged as the command it runs, and a variable set from another is expanded in a second pass. All 28 critic blocks of units-docs were false and pass now; the escapes the old matcher missed or caught are denied. A quoted heredoc body's template literal is no longer a write for any hook | after release |
| A63 | 2026-09-25 | FR-6.5, A29 (F40): `pair` strips comment lines naming the reference in code only; in a document (`.md`, `.rst`, `.txt` and the like) a heading, a bullet or an example's comment is content | after release |
| A64 | 2026-09-25 | FR-17, §8.5 `attest`, `next` (F41): the start hook records each agent's transcript; `next` names an agent whose transcript has been quiet for the policy's `stall_minutes` (15) and how to replace it; `gauntletctl wait` (read-only) returns when one goes quiet or none is left, for the lead to start in the background | after release |
| A65 | 2026-09-25 | FR-12, D37 (F42): `init` records the branch the run started on and `wave` refuses a merge commit on it: units-docs merged its sections into `main`, so the product sat there unreviewed and the pull request would show only the last step | after release |
| A66 | 2026-09-25 | FR-14.3, A7 (F43): an ended run's clock stops at `RUN_ENDED`, and `status` records no lead turn after the end: units-docs read 3.2 of 3 hours the next morning, and the lead's last `status` left the committed log one line behind | after release |
| A67 | 2026-09-26 | FR-6b, A51, §8.5 `controller_only` (F44): during a live run the lead's reads of the installed tooling are denied (file tools, and cat, sed, grep and the like on it); running the controller is not a read. units-3's lead read about 2,000 lines of source before `init`, and it rode in every later call | after release |
| A68 | 2026-09-26 | A64 (F45): every hook call stamps the agent's activity in `.gauntlet/private/activity/`, and `running_agents` takes the latest of that, the transcript and the registration. The run's sandbox hides the transcripts from the sandboxed controller, so A64 alone measured from each agent's start | after release |
| A69 | 2026-09-26 | FR-6.5, A29 (F46): the required tests leave a pair like the other floor trees; they name the loop's `GAUNTLET_OURS`, and a whole-gate critic noticed | after release |
| A70 | 2026-09-26 | FR-6.5, A29, A63 (F47): a comment line goes for a copyright mark, a name with a slash or a dot, a bare name as a path segment, or a comment that is only the bare name; a bare name used as a word stays. units-3's `bytes` library lost 8 doc lines saying "bytes" | after release |
