...
| A44 | 2026-09-24 | FR-17.4, §8.4.1 (Stage E1): a command that moves the run ends by printing the next lines on stderr, the same lines `next` prints; `status` and the hooks' commands do not. The lead stops calling `next` after every event (12 such calls in bytesize-3, 30 in units, each re-reading its whole context); `next` stays for a resume. The command only prints: the lead still decides and dispatches | after release |
